...
$J(\Theta)=-\frac{1}{m} \sum_{i=1}^m \sum_{k=1}^K\left[y^{(i)}_k \log ((h_\Theta (x^{(i)}))_k)+(1-y^{(i)}_k)\log (1 - (h_\Theta(x^{(i)})))\right]+\frac{\lambda}{2m}\sum_{l=1}^{L-1} \sum_{i=1}^{s_l} \sum_{j=1}^{s_{l+1}} ( \Theta_{j,i}^{(l)})^2$
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTU5ODMwOTE2MCwtMzAzNjI0NTE1LDE0MT
